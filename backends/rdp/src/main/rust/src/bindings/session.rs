//! One RDP session: the client, the thread it runs on, and the state Java holds
//! a handle to.
//!
//! The RFB session's twin, and deliberately so — the differences are two, and both
//! are the protocol's: the credentials carry a domain, and they are asked for
//! *before* anything is connected rather than when a server gets round to
//! wanting them.
    ...
    pub fn start(callbacks: Callbacks, config: Config) -> Session {
        ...
        let thread = {
            ...
            std::thread::Builder::new()
                ...
                .expect("spawning the RDP session thread")
        };
