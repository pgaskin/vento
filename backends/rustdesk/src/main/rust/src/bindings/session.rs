...
//! One RustDesk session: the client, the thread it runs on, and the state Java
//! holds a handle to.
...
impl Handler for Bridge {
    ...
    /// There is no user name in this protocol, so a password is the whole of
    ...
    /// Only the id path asks, and the app answers most of them without a person
    /// seeing anything: a key the rendezvous server vouched for is pinned on
    /// sight, and what is worth a prompt is one that has changed since.
    fn trust(&mut self, fingerprint: &str) -> bool {
        self.callbacks.trust_needed(fingerprint);
        self.trust.wait().unwrap_or(false)
    }

    /// The same slot as `trust`, because the two are one question at the
    /// protocol thread's end — go on, or do not — and only one of them can be
    /// outstanding.
    fn unverified(&mut self, why: &str) -> bool {
        self.callbacks.unverified(why);
        self.trust.wait().unwrap_or(false)
    }
}
...
impl Session {
    ...
    pub fn start(callbacks: Callbacks, config: Config) -> Session {
        ...
        let thread = {
            ...
            std::thread::Builder::new()
                ...
                .expect("spawning the RustDesk session thread")
        };
    ...
    pub fn disconnect(&self) {
        if !self.closed.swap(true, Ordering::AcqRel) {
            ...
            // the slots rather than in a socket read, so closing has to wake
